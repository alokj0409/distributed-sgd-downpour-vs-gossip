...
\subsection{Motivation and Key Contributions}
...
\begin{enumerate}
    ...
    \item A software network simulation engine implementing gRPC interceptors (`NetworkSimulatorInterceptor`) for $0\to 100$\,ms artificial latency and $100$\,Mbps bandwidth throttling.
...
Both systems train an identical four-layer CNN (\texttt{CifarCNN}) on CIFAR-10~\cite{krizhevsky2009learning}:
\begin{itemize}
    ...
    \item \textbf{FC}: Linear(1024 $\to$ 512) $\to$ ReLU $\to$ Dropout(0.25) $\to$ Linear(512 $\to$ 10)
\end{itemize}
Total parameters: \textbf{586,250}. The 50,000-sample CIFAR-10 training set is partitioned into $N$ disjoint shards.

\subsection{Communication and Interceptor Layer}
Inter-process communication uses \textbf{gRPC 1.83} over Protocol Buffers 3 (\texttt{proto3}). Raw \texttt{float32} tensors are serialized via \texttt{numpy.tobytes()}. Network simulation is implemented using a custom gRPC client interceptor (\texttt{NetworkSimulatorClientInterceptor}):
\begin{equation}
...
The Parameter Server (\texttt{server.py}) exposes \texttt{PushGradients} and \texttt{PullWeights} RPCs. Upon receiving gradient $\mathbf{g}$ from worker $i$:
\begin{align}
    \mathbf{G} &\leftarrow \mathbf{G} + \mathbf{g}^2 \\
    \mathbf{W} &\leftarrow \mathbf{W} - \frac{\eta}{\sqrt{\mathbf{G} + \epsilon}} \odot \mathbf{g}
\end{align}
where $\eta=0.01, \epsilon=10^{-8}$.

\subsection{Gossip SGD Architecture}
Each peer worker (\texttt{gossip\_worker.py}) acts simultaneously as a gRPC server and client. For worker $i$ at step $k$:
\begin{align}
...
\end{align}
The weight exchange occurs every \texttt{gossip\_every} mini-batches.
...
Table~\ref{tab:config} summarizes the experimental hardware and software setup.
...
Dataset / Model & CIFAR-10 / 4-Layer CNN (586,250 params) \\
Simulated Latencies & $0\text{ ms}, 20\text{ ms}, 50\text{ ms}, 100\text{ ms}$ \\
Simulated Bandwidths & Unlimited ($\infty$), $100\text{ Mbps}$ \\
Gossip Intervals & $\text{gossip\_every} \in \{1, 5, 10\}$ mini-batches \\
Worker Counts & $N \in \{2, 4\}$ \\
...
Table~\ref{tab:convergence} presents per-epoch training metrics under baseline local execution ($N=2, 3$ epochs).

\begin{table}[htbp]
\caption{Baseline Per-Epoch Training Metrics ($N=2$, 3 Epochs)}
