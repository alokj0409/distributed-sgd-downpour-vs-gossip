%% ================================================================
%% IEEE Conference Paper — paper.tex (Master Revised Submission)
...
Prior literature has established theoretical bounds for Decentralized Parallel SGD (D-PSGD)~\cite{lian2017can}. However, our study does not claim theoretical novelty regarding decentralized SGD. Instead, we position this work as a \textbf{controlled systems-level empirical evaluation} of Parameter Server vs.\ pairwise Gossip SGD under systematically varied network latency, bandwidth constraints, communication frequencies, optimizer settings, and worker failures.
...
\begin{enumerate}
    ...
    \item \textbf{RQ2 (Latency \& Bandwidth Crossover)}: Under what empirical latency $\Delta t$ and bandwidth throttling levels does Decentralized Gossip SGD outpace Parameter Server execution?
    \item \textbf{RQ3 (Communication Frequency)}: What is the trade-off between \texttt{gossip\_every} interval ($1, 5, 10$) and wall-clock execution time vs.\ network payload?
    \item \textbf{RQ4 (Communication Concentration \& Faults)}: How does the Communication Concentration Ratio ($CCR$) impact scalability and fault resilience across worker counts $N \in \{2, 4\}$?
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Optimizer Fairness Ablation}: A 4-way optimizer ablation suite isolating architectural network effects from optimizer update rules.
    ...
    \item \textbf{Communication Trade-Off Analysis}: Empirical evaluation of gossip exchange intervals ($\text{gossip\_every} \in \{1, 5, 10\}$), proving a $1.72\times$ runtime speedup and $5\times$ network traffic drop.
    ...
    \item \textbf{Fault-Injection Comparison}: Live process-kill evaluation comparing dynamic peer pruning vs.\ Parameter Server single point of failure (SPOF).
\end{enumerate}
...
While Lian et al.~\cite{lian2017can} establish theoretical convergence bounds and large-scale cluster measurements for D-PSGD, our work explicitly examines a gRPC-based Downpour Parameter Server implementation against pairwise Gossip SGD under controlled latency, bandwidth throttling, communication-frequency sweeps, and live worker-failure conditions on identical hardware.
...
\end{equation}
For Centralized Downpour SGD, $CCR = 1.00$ ($100\%$ of traffic concentrates at the Parameter Server). For Decentralized Gossip SGD, $CCR = \frac{1}{N}$ ($50\%$ for $N=2$, $25\%$ for $N=4$).
...
Table~\ref{tab:ablation} and Fig.~\ref{fig:ablation} report the 4-way optimizer ablation matrix evaluating Downpour SGD and Gossip SGD under identical Momentum SGD ($\mu=0.9$), Adagrad, and Vanilla SGD settings ($N=2, 1$ epoch).

\begin{table}[htbp]
\caption{Optimizer Fairness Ablation Matrix ($N=2$, 1 Epoch)}
\label{tab:ablation}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
...
\caption{Optimizer ablation comparison showing test accuracy (blue) and runtime (orange). Matching both systems under Momentum SGD ($\mu=0.9$) confirms that Downpour's early convergence advantage (+10.03\%) is an architectural property.}
\label{fig:ablation}
\end{figure}

Matching both systems under Momentum SGD ($\mu=0.9$) demonstrates that Downpour SGD achieves **32.09\% test accuracy** vs.\ **22.06\% for Gossip SGD** (+10.03\% advantage). This confirms that Parameter Server early convergence advantage is an architectural property rather than an optimizer artifact.

\subsection{Baseline Convergence Results}
Table~\ref{tab:convergence} presents per-epoch training metrics for both architectures ($N=2, 3$ epochs). Figs.~\ref{fig:train_curves} and~\ref{fig:conv_time} show convergence trajectories.
...
\subsection{Fine-Grained Network Latency Crossover Analysis}
...
\begin{table}[htbp]
\caption{Fine-Grained Network Latency Crossover ($N=2$, 1 Epoch)}
\label{tab:latency}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{ccccc}
...
20 ms  & 87.34 s & \textbf{77.51 s} & 310.8 & \textbf{353.8 (+9.8s)} \\
50 ms  & 134.64 s & \textbf{96.20 s} & 196.2 & \textbf{272.2 (+38.4s)} \\
100 ms & 205.11 s & \textbf{132.17 s} & 126.3 & \textbf{200.3 (+72.9s)} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\caption{Wall-clock execution time vs.\ network latency. Under our evaluated setup, Decentralized Gossip SGD becomes $72.94$\,s faster at $100$\,ms latency.}
\label{fig:latency}
\end{figure}

Under our evaluated setup, Gossip SGD outpaces Downpour SGD as network latency increases, reaching a **72.94-second runtime advantage** at $100\text{ ms}$ latency ($132.17\text{s}$ vs $205.11\text{s}$).

\subsection{Multi-Point Bandwidth Throttling Matrix}
Table~\ref{tab:bandwidth} evaluates performance across bandwidth throttling levels ($\infty, 250, 100\text{ Mbps}$).

\begin{table}[htbp]
\caption{Multi-Point Bandwidth Throttling Matrix ($N=2$, 1 Epoch)}
\label{tab:bandwidth}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
...
Gossip   & 250 Mbps & \textbf{86.25 s} & 304.8 samp/s & Gossip (+0.9s) \\
\midrule
Downpour & 100 Mbps & 127.68 s & 207.5 samp/s & --- \\
Gossip   & 100 Mbps & 132.09 s & 195.7 samp/s & Downpour (+4.4s) \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Table~\ref{tab:scalability_phase5} and Fig.~\ref{fig:scalability} report parallel speedup $S(N) = \frac{T_1}{T_N}$ and efficiency $E(N) = \frac{S(N)}{N}$.
...
Gossip SGD achieves higher measured parallel efficiency at $N=4$ under the optimized exchange configuration ($57.45\%$ vs.\ $51.87\%$).
...
Matching both systems under Momentum SGD ($\mu=0.9$) demonstrates that Downpour SGD achieves $32.09\%$ Epoch-1 accuracy vs.\ $22.06\%$ for Gossip SGD (+10.03\% advantage). This proves that Parameter Server early convergence advantage is an architectural property (centralized gradient aggregation per batch) rather than an Adagrad optimizer artifact.
...
The Communication Concentration Ratio ($CCR$) highlights the structural trade-off. Downpour maintains $CCR=1.00$ ($100\%$ of cluster traffic hits the single PS), creating bandwidth saturation under constrained links. Gossip distributes network demand ($CCR = 1/N$), avoiding single-node NIC bottlenecks.

\subsection{Fault Tolerance: Fundamental Architectural Difference}
Gossip SGD's peer-pruning mechanism requires no external coordination and degrades gracefully. Downpour SGD has no recovery path when its PS is killed. This SPOF characteristic is a structural property of the centralized paradigm.
...
This paper presented a controlled systems-level empirical comparison of Downpour SGD (Parameter Server) and Gossip SGD (Peer-to-Peer) across optimizer fairness, latency/bandwidth constraints, exchange frequency, communication concentration ($CCR$), and live fault injection.

Principal findings: Under our evaluated configuration, Downpour SGD trains faster with lower network overhead under unconstrained local conditions. However, as network latency increases to $100$\,ms, Decentralized Gossip SGD becomes $72.94$\,s faster. Optimizing gossip interval ($\text{gossip\_every}=5$) cuts network payload by $80\%$ and delivers $57.45\%$ parallel efficiency at $N=4$ workers.
