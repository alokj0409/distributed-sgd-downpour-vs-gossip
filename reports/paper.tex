...
Principal findings: While Downpour SGD provides fast initial convergence on unconstrained local loopback networks, Decentralized Gossip SGD is the superior architectural choice under \textbf{bandwidth constraints ($\le 100\text{ Mbps}$)} and \textbf{high network latencies ($\ge 50\text{ ms}$)}. Optimizing exchange frequency to $\text{gossip\_every}=5$ cuts network payload by $80\%$ and delivers $57.45\%$ parallel efficiency at $N=4$ workers.
